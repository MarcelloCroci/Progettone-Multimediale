...
//RECENSIONI IS
app.get("/recensioni/:id_utente", async (req, res) => {
  const { id_utente } = req.params;
  console.log(`Richiesta ricevuta per recensioni dell'utente con ID: ${id_utente}`);
  ...
  `SELECT 
      ...
      r.voto,
      r.id_utente,
      ri.nome AS titolo_risorsa
   FROM 
      recensione r
   JOIN 
      risorsa ri ON ri.id_risorsa = r.id_risorsa
   WHERE 
      r.id_utente = $1
   ORDER BY 
      r.data_r DESC`,
...
app.post("/recensione", async (req, res) => {
  ...
  const { userId, risId, titolo_r, testo, valutazione } = req.body;
  
  if (!userId || !risId || !titolo_r || !testo || !valutazione) {
    return res.status(400).json({ error: "Tutti i campi sono obbligatori" });
  }
  
  try {
     const movieCheck = await pool.query("SELECT * FROM risorsa WHERE id_risorsa = $1", [risId]);
    if (movieCheck.rows.length === 0) {
      return res.status(404).json({ error: "Risorsa non trovata" });
    }

    const userCheck = await pool.query("SELECT * FROM utente WHERE id_utente = $1", [userId]);

    if (userCheck.rows.length === 0) {
      return res.status(404).json({ error: "Utente non identificato" });
    }
      
    if (!Number.isInteger(valutazione) || valutazione < 1 || valutazione > 10) {
    ...
    const result = await pool.query(
      "INSERT INTO recensione (id_utente, id_risorsa, titolo_r, testo_r, valutazione) VALUES ($1, $2, $3, $4, $5) RETURNING *",
      [userId, risId, titolo_r, testo, valutazione]
